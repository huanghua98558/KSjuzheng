...
async def create_collect_pool_auth_code(request: Request):
    body = await request.json()
    return _success({"auth_code": body.get("auth_code"), "stored_in": "collect_pool.auth_code"})


@router.put("/collect-pool/auth-code/{auth_id}")
async def update_collect_pool_auth_code(auth_id: int, request: Request):
    body = await request.json()
    return _success({"id": auth_id, "auth_code": body.get("auth_code")})


@router.delete("/collect-pool/auth-code/{auth_id}")
async def delete_collect_pool_auth_code(auth_id: int):
