...
def target_overlaps_source(args, engine) -> bool:
    url = engine.url
    backend = (url.get_backend_name() or "").lower()
    if "mysql" not in backend:
        return False
    target_host = (url.host or "").strip().lower()
    target_port = int(url.port or 3306)
    target_db = (url.database or "").strip().lower()
    source_host = str(args.mysql_host or "").strip().lower()
    source_port = int(args.mysql_port or 3306)
    source_db = str(args.mysql_database or "").strip().lower()
    return (
        target_host == source_host
        and target_port == source_port
        and target_db == source_db
    )
...
def ensure_target_schema(engine) -> None:
    ...
    with engine.begin() as conn:
        ...
        cols = {row[1] for row in conn.exec_driver_sql("PRAGMA table_info(fluorescent_members)").fetchall()}
        if "avatar" not in cols:
            conn.exec_driver_sql("ALTER TABLE fluorescent_members ADD COLUMN avatar VARCHAR(500)")
        if "in_limit" not in cols:
            conn.exec_driver_sql(
                "ALTER TABLE fluorescent_members ADD COLUMN in_limit BOOLEAN NOT NULL DEFAULT 0"
            )
        table = conn.exec_driver_sql(
            "SELECT sql FROM sqlite_master WHERE type='table' AND name='drama_link_statistics'"
        ).scalar()
        if table:
            cols = {row[1] for row in conn.exec_driver_sql("PRAGMA table_info(drama_link_statistics)").fetchall()}
            if "task_type" not in cols or "uq_dls_org_url" in table:
                conn.exec_driver_sql("ALTER TABLE drama_link_statistics RENAME TO drama_link_statistics_old")
                conn.exec_driver_sql(
                    """
                    CREATE TABLE drama_link_statistics (
                        organization_id INTEGER NOT NULL,
                        drama_url VARCHAR(1000) NOT NULL,
                ...
                old_cols = {row[1] for row in conn.exec_driver_sql("PRAGMA table_info(drama_link_statistics_old)").fetchall()}
                task_expr = "task_type" if "task_type" in old_cols else "NULL"
                conn.exec_driver_sql(
                    f"""
                    INSERT INTO drama_link_statistics (
                        organization_id, drama_url, drama_name, task_type,
                        execute_count, success_count, failed_count, account_count,
                        last_executed_at, id, created_at, updated_at
                    )
                    SELECT
                        organization_id, drama_url, drama_name, {task_expr},
                        execute_count, success_count, failed_count, account_count,
                        last_executed_at, id, created_at, updated_at
                    FROM drama_link_statistics_old
                    """
                )
                conn.exec_driver_sql("DROP TABLE drama_link_statistics_old")
                conn.exec_driver_sql(
                    "CREATE INDEX IF NOT EXISTS ix_drama_link_statistics_organization_id "
                    "ON drama_link_statistics (organization_id)"
                )
                conn.exec_driver_sql(
                    "CREATE INDEX IF NOT EXISTS ix_drama_link_statistics_drama_url "
                    "ON drama_link_statistics (drama_url)"
                )
                conn.exec_driver_sql(
                    "CREATE INDEX IF NOT EXISTS ix_drama_link_statistics_task_type "
                    "ON drama_link_statistics (task_type)"
                )
                conn.exec_driver_sql(
                    "CREATE INDEX IF NOT EXISTS ix_drama_link_statistics_last_executed_at "
                    "ON drama_link_statistics (last_executed_at)"
                )
    ...
    for row in fluorescent_rows:
        ...
        fluor_objects.append(
            FluorescentMember(
                ...
                avatar=clip(row.get("member_head"), 500),
                fans_count=to_int(row.get("fans_count")),
                in_limit=bool(to_int(row.get("in_limit"))),
                broker_name=clip(row.get("broker_name"), 100),
...
def import_high_income_links(db, conn, default_org_id: int) -> None:
    rows = fetch_all(
        conn,
        """
        SELECT
            h.title AS drama_name,
            k.url AS drama_url,
            COUNT(*) AS reference_count,
            COUNT(DISTINCT k.uid) AS account_count
        FROM spark_highincome_dramas h
        JOIN kuaishou_urls k ON k.name = h.title
        WHERE k.url IS NOT NULL AND k.url <> ''
        GROUP BY h.title, k.url
        """,
    )
    objects: list[HighIncomeDramaLink] = []
    seen: set[tuple[str, str]] = set()
    for row in rows:
        name = clip(row.get("drama_name"), 200)
        url = extract_first_url(row.get("drama_url")) or clip(row.get("drama_url"), 1000)
        ...
        objects.append(
            HighIncomeDramaLink(
                ...
                reference_count=to_int(row.get("reference_count")),
                account_count=to_int(row.get("account_count")),
            )
        )
    bulk_insert(db, "high_income_drama_links", objects)
...
def import_drama_link_statistics(db, conn, org_map: dict[int, int]) -> None:
    rows = fetch_all(
        conn,
        """
        SELECT
            ...
            ts.uid,
            ts.drama_name,
            COALESCE(ts.task_type, 'unknown') AS task_type,
            ts.drama_link,
            ts.status,
            ts.created_at,
            ts.start_time,
        ...
        LEFT JOIN kuaishou_accounts ka ON ka.uid = ts.uid
        WHERE COALESCE(ts.task_type, '') <> 'drama_favorite'
          AND ts.drama_name IS NOT NULL
          AND ts.drama_name <> ''
        """,
        (DEFAULT_ORG_SOURCE_ID,),
    )
    ...
    for row in rows:
        org_id = org_map.get(to_int(row.get("source_org_id"), DEFAULT_ORG_SOURCE_ID))
        raw_name = str(row.get("drama_name") or "")[:200]
        name = raw_name.strip()
        task_type = clip(row.get("task_type"), 50) or "unknown"
        if not org_id or not name:
            continue
        key = (org_id, raw_name, task_type)
        ...
        if row.get("uid"):
            current["uids"].add(str(row.get("uid")))
        url = extract_first_url(row.get("drama_link"))
        if url:
            current["urls"][url] = current["urls"].get(url, 0) + 1
        latest = row.get("created_at") or row.get("start_time") or row.get("end_time")
        if latest and (
            current["last_executed_at"] is None or latest > current["last_executed_at"]
        ):
            current["last_executed_at"] = latest

    objects: list[DramaLinkStatistic] = []
    for item in grouped.values():
        urls = item.pop("urls")
        uids = item.pop("uids")
        drama_url = max(urls.items(), key=lambda pair: pair[1])[0] if urls else ""
        objects.append(
            DramaLinkStatistic(
                **item,
                drama_url=drama_url,
    ...
    bulk_insert(db, "drama_link_statistics", objects)
...
def external_platform(url: str) -> str:
    lowered = url.lower()
    if "kuaishou.com" in lowered or "yximgs.com" in lowered or "kwai" in lowered:
        return "kuaishou"
    if "douyin.com" in lowered or "iesdouyin.com" in lowered:
        return "douyin"
    if "xigua" in lowered:
        return "xigua"
    if "weixin.qq.com" in lowered or "wechat" in lowered:
        return "wechat"
...
def import_external_url_stats(db, conn, org_map: dict[int, int]) -> None:
    rows = fetch_all(
        conn,
        """
        SELECT
            ...
            ts.drama_link,
            COUNT(*) AS reference_count,
            MAX(ts.created_at) AS last_seen_at
        FROM task_statistics ts
        LEFT JOIN kuaishou_accounts ka ON ka.uid = ts.uid
        WHERE ts.drama_link IS NOT NULL AND ts.drama_link <> ''
        GROUP BY source_org_id, ts.drama_link
        """,
        (DEFAULT_ORG_SOURCE_ID,),
    )
    ...
    for row in rows:
        ...
        current = grouped.setdefault(
            key,
            {
                "organization_id": org_id,
                "url": url,
                "source_platform": external_platform(url),
                "reference_count": 0,
                "last_seen_at": row.get("last_seen_at"),
            },
        )
        current["reference_count"] += to_int(row.get("reference_count"))
        if row.get("last_seen_at") and (
            current["last_seen_at"] is None or row.get("last_seen_at") > current["last_seen_at"]
        ):
            current["last_seen_at"] = row.get("last_seen_at")

    objects = [ExternalUrlStat(**item) for item in grouped.values()]
    bulk_insert(db, "external_url_stats", objects)
...
def run_import(args) -> None:
    engine = init_engine()
    if target_overlaps_source(args, engine):
        raise RuntimeError(
            "Refusing to import into the same MySQL database as the Huoshijie source. "
            "This importer assumes separate source/target tables and can wipe source data."
        )
    ...
    try:
        ...
        with Session() as db:
            ...
            import_high_income_links(db, conn, fallback_org_id)
            ...
            import_drama_link_statistics(db, conn, org_map)
            import_external_url_stats(db, conn, org_map)
